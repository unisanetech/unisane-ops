...
const CONFIG_IMPORT = "import { defineUnisaneOps, defineUnisaneProject } from 'unisane/config';";
...
function ensureConfigImport(source: string): string {
  ...
  return `import { defineUnisaneOps } from 'unisane/config';\n${source}`;
}
...
export function updateOpsConfigSource(args: {
  ...
  throw new Error(
    '[UNISANE_OPS_CONFIG_NOT_OWNED] The canonical Ops block is not tool-owned. Update the named ops export directly or re-run ops init after removing the manual declaration.',
  );
}
