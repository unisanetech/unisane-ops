...
export async function runMigrateGrowthConfig(
  ...
  if (parsed.positionals.length > 0) {
    throw new Error(
      `[OPS_CLI_ARGUMENT_UNKNOWN] ops migrate growth-config: ${parsed.positionals.join(' ')}`,
    );
  }
  ...
  return commandResult(context, {
    ...
    nextActions: [
      'Delete the retired input after review, then run `unisane connect google` and `unisane check`.',
    ],
  });
}
