...
export async function runDisconnect(context: PackCommandContext): Promise<PackCommandResult> {
  ...
  if (!growth) {
    throw new Error(
      '[GROWTH_CAPABILITY_NOT_SELECTED] Run `unisane add growth` before managing a Growth connection.',
    );
  }
  ...
  if (!confirmed) {
    return commandResult(context, {
      ...
      nextActions: [
        `Re-run \`unisane disconnect google --environment ${environmentId} --connection ${connectionId} --yes\` after reviewing the consequences.`,
      ],
    });
  }
  if (!context.runtime) {
