...
export async function runOpsInit(context: PackCommandContext): Promise<PackCommandResult> {
  ...
  if (parsed.positionals.length > 0) {
    throw new Error(`[OPS_CLI_ARGUMENT_UNKNOWN] ops init: ${parsed.positionals.join(' ')}`);
  }
  ...
  return commandResult(context, {
    ...
    nextActions: config.capabilities.growth
      ? ['Run `unisane connect google` to connect the first Growth provider.']
      : ['Run `unisane add growth` to select Growth capabilities.'],
  });
}
