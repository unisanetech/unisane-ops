...
test('plain projects use one init, connect, and aggregate check lifecycle', () => {
  ...
  assert.equal(JSON.parse(initialized.stdout).command, 'core.init');
...
test('Framework projects retain their default config and receive one named Ops block', () => {
  ...
  const initialized = run(['init', '--yes', '--json'], root);
  assert.equal(initialized.status, 0, initialized.stderr || initialized.stdout);
  const source = readFileSync(resolve(root, 'unisane.config.ts'), 'utf8');
...
test('new adoption writes the same canonical Growth intent through init', () => {
  ...
  const initialized = run(['init', '--growth', '--mode', 'new', '--yes', '--json'], root);
  assert.equal(initialized.status, 0, initialized.stderr || initialized.stdout);
  const source = readFileSync(resolve(root, 'unisane.config.ts'), 'utf8');
...
test('the retired combined-host ops prefix fails closed', () => {
  const result = run(['ops', 'init'], packageRoot);
  assert.equal(result.status, 1);
  assert.match(result.stderr, /UNISANE_OPS_COMMAND_UNKNOWN/);
});
...
test('the one-shot migrator converts retired intent without copying credentials', () => {
  ...
  const migrated = run(
    ['migrate', 'growth-config', '--input', 'retired-growth.mjs', '--yes', '--json'],
    root,
  );
