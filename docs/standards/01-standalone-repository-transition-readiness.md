...
The source-bound policy keeps six exact preconditions open:
...
3. accept and prove the console/repository Node support matrix
4. approve registry versions, visibility, and access
5. prove an isolated consumer lock, TypeScript/browser build, runtime, CSS/assets, and one
   React/ReactDOM singleton from immutable artifacts
6. obtain explicit legal, security, publication, and consumer-conversion authority
...
| OPS-R03 | open                                           | local shadow                | console retains exactly two private UI workspace edges; the producer is technically certified and DataTable CSS is composed and bundled, but Node, registry/version access, isolated consumer, and legal/release authority preconditions remain open                                | UI and Ops release owners; exact six-blocker consumer verifier, approved immutable coordinates, and clean external consumer proof |
