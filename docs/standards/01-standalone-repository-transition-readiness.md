...
TypeScript, ESLint, Prettier, Vitest, Turbo, and Changesets. Current package engines instead span
Node `>=18.17.0`, `>=20.0.0`, and `>=22.13.0`.

**Owner decision:** Confirm the supported Node/runtime matrix and whether package engines match the
repository toolchain floor or a deliberately broader tested consumer floor. Do not silently copy
either side. Record the result in root policy, package metadata, CI matrix, images, and
clean-install proof.
...
inventory. At Task `T-0789a7ed` it records 152 UI/DataTable import declarations across 53 console
files: 142 `@unisane/ui` declarations across 50 files and ten `@unisane/data-table` declarations
across nine files. The inventory contains zero computed loaders and rejects nonliteral `import()`,
...
browser composition root imports the Material Symbols and UI stylesheets but does not yet import
`@unisane/data-table/styles.css`; that missing import is a blocker, not an implicit fallback.
...
The source-bound policy keeps seven exact preconditions open:
...
3. add the DataTable stylesheet once at the browser composition root and prove emitted CSS and
   visual behavior
4. accept and prove the console/repository Node support matrix
5. approve registry versions, visibility, and access
6. prove an isolated consumer lock, TypeScript/browser build, runtime, CSS/assets, and one
   React/ReactDOM singleton from immutable artifacts
7. obtain explicit legal, security, publication, and consumer-conversion authority
...
| OPS-R03 | open                                           | local shadow                | console retains exactly two private UI workspace edges; the producer is technically certified, but DataTable CSS, Node, registry/version access, isolated consumer, and legal/release authority preconditions remain open                                                           | UI and Ops release owners; exact seven-blocker consumer verifier, approved immutable coordinates, and clean external consumer proof |
